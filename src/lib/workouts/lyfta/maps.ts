// Lyfta returns equipment / body-part / muscle metadata as JSON-encoded arrays
// of numeric ids. These lookup tables (from the Lyfta API docs) map ids to names.
...
export function namesFor(ids: string[], table: Record<string, string>): string[] {
  return ids.map((id) => table[id]).filter((n): n is string => Boolean(n));
}
